...
# Helper for Suffix-Aware Names (Thomas Jr., Walker III, etc.)
...
try:
    ...
    if main_page == "Draft Room":
        ...
        if sub_page == "Dashboard":
            ...
            c3.metric("Avg ROI", f"{owner_draft['ROI Score'].mean():.1f}")
            ...
            # --- ROW 2: TEAM RELIANCE (FIXED VISUALS) ---
            ...
            # Sort so most drafted are at the top
            team_data = team_data.sort_values('Picks', ascending=True)

            fig_teams = px.bar(
                team_data, x='Picks', y='Team', orientation='h',
                height=900, text='Picks', 
                color='Picks', color_continuous_scale='GnBu',
                title="Career Picks by NFL Franchise"
            )
            # Layout fixes for overlapping text
            fig_teams.update_layout(
                margin=dict(l=150), # Add space for the team names
                yaxis=dict(dtick=1), # Force a label for every single team
                xaxis=dict(title="Number of Times Drafted")
            )
